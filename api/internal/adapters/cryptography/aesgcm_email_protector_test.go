...
func TestDecryptGCMRejectsShortCiphertext(t *testing.T) {
	if _, err := decryptGCM([]byte("short"), []byte("01234567890123456789012345678901")); err == nil {
...
func TestGCMRejectsInvalidAESKeys(t *testing.T) {
	...
	if _, err := encryptGCM([]byte("ana@example.com"), invalidKey); err == nil {
		t.Fatal("expected encryption with an invalid key to fail")
	}
	if _, err := decryptGCM([]byte("ciphertext"), invalidKey); err == nil {
...
func TestAESGCMEmailProtectorRejectsInvalidDecryptedEmail(t *testing.T) {
	...
	ciphertext, err := encryptGCM([]byte("not-an-email"), []byte("01234567890123456789012345678901"))
	if err != nil {
		t.Fatal(err)
	}
