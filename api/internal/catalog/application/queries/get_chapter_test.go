...
func TestGetChapter(t *testing.T) {
	...
	got, err := queries.NewGetChapter(catalogRepository{chapter: want}).Execute(context.Background(), "gn", 9)
	if err != nil || got.BookID() != want.BookID() {
		t.Fatalf("unexpected result: %+v, %v", got, err)
	}
}
func TestGetChapterNotFound(t *testing.T) {
	_, err := queries.NewGetChapter(catalogRepository{err: domain.ErrChapterNotFound}).Execute(context.Background(), "xx", 1)
